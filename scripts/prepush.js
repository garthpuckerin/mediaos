...
if (affected.length === 0) {
  // Nothing in packages changed; exit cleanly
  process.exit(0);
}

// 2) Lint only affected packages
for (const ws of affected) {
  // eslint per workspace path to avoid requiring a lint script in each package.json
...
// 3) Type-check affected packages (tsconfig at package level)
for (const ws of affected) {

...
Configure a SAB-only build, key environment options, and a quick smoke test to validate core flows.
...
2. Under SABnzbd:
   ...
   - Category: optional (for example `tv`)
...
- If uploads/enqueues work but queue is empty, recheck SAB Category and API key; verify SAB is not paused.
- If Verify does not update instantly, the item detail verify panel auto-refreshes briefly; for immediate results, use Verify/Verify again.
